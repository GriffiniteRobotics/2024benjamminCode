...
public class Robot extends TimedRobot {
  ...
  public void robotPeriodic() {
    ...
    SmartDashboard.putNumber("FRONT_LEFT_SPEED", FRONT_LEFT_SPEED.getEncoder().getPosition());
    SmartDashboard.putNumber("FRONT_RIGHT_SPEED", FRONT_RIGHT_SPEED.getEncoder().getPosition());

    SmartDashboard.putNumber("BACK_LEFT_SPEED", BACK_LEFT_SPEED.getEncoder().getPosition());
    SmartDashboard.putNumber("BACK_RIGHT_SPEED", BACK_RIGHT_SPEED.getEncoder().getPosition());
     ...
     */
     ...
     switch ("LTFS") {
      case "L":
        ...
        if (timer.get() < 2) {// 2 secs = 4 ft
          speeds = ChassisSpeeds.fromFieldRelativeSpeeds(1.2, 0, INIT_CLAMPED_YAW, Rotation2d.fromDegrees(navx.getYaw()));
        } 
  ...
  public void teleopPeriodic() {
    ...
    if(XBOX_ONE.getYButton()){
      ...
      if(note_in_low == false && note_in_high == false){
        if(RunIntake){
          intake.set(ControlMode.PercentOutput, -.26);
        }else{
          intake.set(ControlMode.PercentOutput, 0);
        }
      }
    ...
    FRONT_LEFT_SPEED.set(frontLeftOptimized.speedMetersPerSecond/2);
    FRONT_RIGHT_SPEED.set(frontRightOptimized.speedMetersPerSecond/2);
    BACK_LEFT_SPEED.set(backLeftOptimized.speedMetersPerSecond/2);
    BACK_RIGHT_SPEED.set(backRightOptimized.speedMetersPerSecond/2);
    ...
    if(XBOX_ONE.getXButtonPressed() == true && Shooting == false && note_in_high){
      ...
      //Shooting_Timer.reset();
      Shooting = true;
    }else if (Shooting_Timer.get() >=.5 && Shooting_Timer.get() < 1.5 && Shooting == true){
      System.out.println("step2 Feeder and Shooting");
      feeder.set(ControlMode.PercentOutput, 0.3);
      shooter1.set(ControlMode.PercentOutput, 0.7);
      shooter2.set(ControlMode.PercentOutput, 0.7);
    }else if (Shooting_Timer.get() >= 1.5 && Shooting == true) {
      System.out.println("Stopping");
      feeder.set(ControlMode.PercentOutput, 0);
      shooter1.set(ControlMode.PercentOutput, 0);
      shooter2.set(ControlMode.PercentOutput, 0);
  ...
  System.out.println("Danger danger !!!!!!!!!!!!!!!!count 2");*/
    ...
    if(gamepad.getLeftBumper() && climb1.getSelectedSensorPosition()<=171000){ //103248.000000
      climb1.set(ControlMode.PercentOutput,0.5);
    }else if( gamepad.getLeftTriggerAxis() > 0.2 /*&& climb1.getSelectedSensorPosition()>=INIT_CLIMB1*/){//INIT_CLIMB1
    ...
    if(gamepad.getRightTriggerAxis() > 0.2 /*&& climb2.getSelectedSensorPosition()>=INIT_CLIMB2*/){//INIT_CLIMB2
      climb2.set(ControlMode.PercentOutput,-0.5);
      System.out.println("Climbing 2...");
    }else if(gamepad.getRightBumper() && climb2.getSelectedSensorPosition()<=183500){//95692.000000//183560
      climb2.set(ControlMode.PercentOutput,0.5);
    }else{
